...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_forms__WEBPACK_IMPORTED_MODULE_2__["default"])(calcSum);
  Object(_modules_mask__WEBPACK_IMPORTED_MODULE_3__["default"])('[name="phone"]');
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name="name"]');
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_4__["default"])('[name="message"]');
  Object(_modules_showMoreStyles__WEBPACK_IMPORTED_MODULE_5__["default"])('.button-styles', '#styles .row');
  Object(_modules_calc__WEBPACK_IMPORTED_MODULE_6__["default"])('#size', '#material', '#options', '.promocode', '.calc-price', calcSum);
...
var calc = function calc(size, material, options, promocode, result, calcSum) {
  var sizeBlock = document.querySelector(size),
      materialBlock = document.querySelector(material),
      optionsBlock = document.querySelector(options),
      promocodeBlock = document.querySelector(promocode),
      resultBlock = document.querySelector(result);
  var sum = 0; // Расчет по значениям, заданным в верстке (атрибут value)
  // const calcFunc = () => {
  ...
  function changeParam(event, elem) {
    elem.addEventListener(event, function (e) {
      ...
      function calcFunc(dataFromDB) {
        ...
        for (var key in dataFromDB[selectedField]) {
...
var filter = function filter(noPortfolio) {
  var menu = document.querySelector('.portfolio-menu'),
      ...
      noCards = document.querySelector('.portfolio-no'); // скрываем все карточки
  ...
  function showCards(clickedClass) {
    // скрываем все
    closeCards(); // проверяем совпадение со списком из main.js
    ...
    try {
      for (var _iterator = noPortfolio[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var i = _step.value;

        // если совпадение есть, то показывается блок без фотографий (с надписью)
        if (clickedClass == i) {
          noCards.style.display = 'block';
          noCards.classList.add('animated', 'fadeIn'); // иначе показываются карточки с таким же классом, как и в кнопке
        } else {
          cards.forEach(function (card) {
  ...
  menu.addEventListener('click', function (e) {
    var target = e.target; // убираем класс активности у всех кнопок
    ...
    if (target && target.tagName == 'LI') {
      ...
      showCards(target.className); // назначаем кнопке класс активности
...
var forms = function forms(sum) {
  ...
  uploads.forEach(function (upload) {
    upload.addEventListener('input', function () {
      var dots;
      var name = upload.files[0].name.split('.');
  ...
  function clearInputs() {
    ...
    uploads.forEach(function (upload) {
      return upload.previousElementSibling.textContent = 'Файл не выбран';
    });
  ...
  forms.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var formData = new FormData(item); // добавляем сумму, расчитанную в модуле calc
...
var modals = function modals() {
  ...
  var btnPressed; // содержит информацию о том, было ли открыто окно с подарком

  var giftOpened; // Определение ширины полосы прокрутки
  ...
  function changeElementMarginScroll(selector) {
    ...
    var elemRight = +getComputedStyle(elem).right.replace(/\D/g, '') + calcScroll();

    if (checkOpenModal()) {
      elem.style.right = "".concat(elemRight, "px");
    } else {
      elem.style.right = '';
    }
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  // cards и styles использовались для случая подгрузки из верстки
  var btn = document.querySelector(trigger); // cards = document.querySelectorAll(styles);
  // Вариант для отображения карточек, скрытых в верстке
  // cards.forEach(card => {
  ...
  // });
  // Вариант для подгрузки карточек из JSON-файла
